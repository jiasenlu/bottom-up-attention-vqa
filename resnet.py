...
class resnet(nn.Module):
  def __init__(self, data_path, _num_layers=101, _fixed_block=1, pretrained=True):
    super(resnet, self).__init__()
    self._num_layers = _num_layers
    self._fixed_block = _fixed_block
    self.pretrained = pretrained
    self.model_path = pth.join(data_path, 'imagenet_weights',
                                'resnet' + str(_num_layers) + '.pth')
    ...
    self.v_dim = self.resnet.fc.in_features
